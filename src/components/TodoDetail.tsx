...
import { getTodoItemPath } from "../utils/apiPath";
...
const TodoDetail: React.FC = () => {
  ...
  return <div className="ml-12 p-5 flex-grow bg-blue-50">
    <div className="text-2xl">Todo Detail</div>
    {data ? <div>
    Content: {data.content} <br/>
    Completed: <input type="checkbox" checked={data.completed} onChange={async () => {
      await axios.patch(getTodoItemPath(data.id), {
        completed: !data.completed
      });
      mutate(getTodoItemPath(data.id))
    }}/>
  </div> : <div>No item to display</div>}</div>;
};
